...
with open(os.path.join(model.Path, "log.txt"), "w") as log_file:
    ...
    # Load stations file
    stations_temp = ClimateStations(
        os.path.join(model.Path, model.desc["StationsTemp"])
    )
    stations = ClimateStations(os.path.join(model.Path, model.desc["Stations"]))

    progress.setConsoleInfo("Reading old climate data...")
    pcp_juliandates, first_pcp_date, last_pcp_date, pcp_array = stations.readPcpFiles(
        log_file
    )
    (
        tmp_juliandates,
    ...
    if new_pcp_files:
        ...
        new_pcp_juliandates, new_pcp_array = ZonalStats(
            pcp_startdate,
            new_pcp_enddate,
            os.path.join(model.Path, model.desc["Shapefile"]),
            model.desc["SubbasinColumn"],
            new_pcp_files,
            subcatchmap_res,
            None,
            correct_factor,
    ...
    if new_tmax_files and new_tmin_files:
        ...
        new_tmax_juliandates, new_tmp_max_array = ZonalStats(
            tmp_startdate,
            new_tmax_enddate,
            os.path.join(model.Path, model.desc["Shapefile"]),
            model.desc["SubbasinColumn"],
        ...
        if tmin_var_ECMWF in new_tmin_files[0]:
            ...
            correct_number = None
        new_tmin_juliandates, new_tmp_min_array = ZonalStats(
            tmp_startdate,
            new_tmin_enddate,
            os.path.join(model.Path, model.desc["Shapefile"]),
            model.desc["SubbasinColumn"],
